...
import {
  GET_CAREER,
  INSEERT_CAREER,
} from './../../../../graphql/student/worksheets/career/Career';
...
const Approach = (props) => {
  ...
  React.useEffect(() => {
    props.client
      .query({
        query: GET_CAREER,
        variables: { usersub: props.sub, subid: props.subid },
      })
      .then((res) => {
        const data = res.data;
        if (data) {
  ...
  const summaryTable = () => {
    return contents1.map((content, index) => (
      <TableRow key={index}>
        <TableCell className={classes.tablecell1}>{content}</TableCell>
        <TableCell className={classes.tablecell1}>
          <Textfield
            id={content}
            value={approach && JSON.parse(approach).goals[index]}
          />
        </TableCell>
      </TableRow>
    ));
                ...
                {contents2.map((content, index1) => (
                  <TableRow key={index1}>
                    ...
                    <TableCell className={classes.tablecell2}>
                      <Textfield
                        id={`${params[index1]}-${index}`}
                        value={
                          approach && JSON.parse(approach).jobs[index][index1]
                        }
                      />
                    </TableCell>
                  </TableRow>
      ...
      <div>
        ...
        <div className="center">
          <div style={{ width: '80%', textAlign: 'center' }}>
            <h2 className="center">
              5.1 Jobs exploration - Introspective approach
            </h2>
        ...
        <div className="center">
          <div className="p-60">
            <p className="text_center">
              Now you know who you are and who you want to become. The next step
              is to learn about the outside world and to introduce yourself to
              the most relevant careers and majors.
            </p>
            <div className="line"></div>
            <p className="mt-5">
              1. Go back to{' '}
              <Linkto
                to={`${props.pathname}/${props.worksheets[3].worksheet_url}/${props.lifes[5].url}`}
                value="worksheets 4.6"
              />{' '}
              and copy-paste the ten-word version of your ‘To be’, ‘To have’ and
              ‘To give’ life goals here.
            </p>
            <Wrapper
              components={[
                <Table className={classes.table} aria-label="caption table">
                  <TableBody>{summaryTable()}</TableBody>
                </Table>,
                <Comment
                  type={props.type}
                  client={props.client}
            ...
            <p className="mt-5">
              ...
              Don’t worry about the reasons for these choices. Then, evaluate
              each of them on how these jobs satisfy your ‘To be’, ‘To have’,
              ‘To do’ life goals. Search online and find out the top two
              relevant college degrees to the job.
            </p>
            {jobsTable()}
          </div>
        </div>
